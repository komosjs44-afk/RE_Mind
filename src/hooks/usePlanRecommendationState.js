...
export function usePlanRecommendationState({ baseRecommendations, showToast, closeToast }) {
  ...
  // Load persisted state async on mount
  useEffect(() => {
    loadInterventionState(storageKey).then((loaded) => {
  ...
  useEffect(() => {
    const tick = setInterval(() => setCurrentTime(Date.now()), 60 * 1000);
  ...
  const moveRecommendation = async (id, nextTime) => {
    ...
    showToast({ message: `${nextTime}으로 이동했습니다.` });
  };
  ...
  const dismissRecommendation = (id) => {
    const previousState = stateRef.current[id] || {};
