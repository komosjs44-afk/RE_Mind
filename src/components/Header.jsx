  ...
  return (
    <>
      ...
      <header className="relative flex items-center justify-between border-b border-border bg-bg/90 px-5 py-3.5 backdrop-blur-md">
        ...
        <button
          className="relative grid h-[38px] w-[38px] place-items-center rounded-full bg-bg text-text-secondary"
          aria-label="알림"
          aria-expanded={open}
        ...
        {open && (
          <div className="absolute right-4 top-[68px] z-40 w-[292px] rounded-lg border border-border bg-card p-3.5 shadow-lg">
            ...
            {notifications.length === 0 ? (
              <p className="rounded-lg bg-bg px-3 py-4 text-center text-[12px] font-medium text-text-secondary">
                현재 확인할 알림이 없습니다.
              </p>
            ) : (
              <div className="flex flex-col gap-2">
                {notifications.map((item) => (
                  <article key={item.id} className="rounded-lg bg-bg px-3 py-2.5">
