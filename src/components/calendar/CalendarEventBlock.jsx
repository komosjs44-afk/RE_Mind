export default function CalendarEventBlock({ event, style }) {
  const focusDemand = event.risk ? '높음' : event.title.includes('집중') ? '보통' : '낮음';

  return (
    <article
      className={`z-20 min-h-0 rounded-[18px] border bg-card px-3 py-2 shadow-sm ${event.risk ? 'border-risk/35 bg-risk-soft/60' : 'border-border'}`}
      style={style}
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {event.risk && (
          <span className="flex-none rounded-full bg-risk px-2 py-0.5 text-[10px] font-semibold text-white">위험</span>
        )}
      </div>
      <div className="mt-1.5 flex flex-wrap gap-1.5">
        <Meta label="집중 부담" value={focusDemand} risk={focusDemand === '높음'} />
      </div>
    </article>
  );
}
