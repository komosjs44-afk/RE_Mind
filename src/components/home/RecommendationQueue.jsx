  ...
  return (
    <article className="rounded-[18px] bg-bg">
        ...
        <div className="border-t border-border/50 px-3 pb-3 pt-2">
          ...
          {item.alternatives?.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1.5">
              <span className="text-[11px] font-semibold text-text-secondary self-center">대안:</span>
              {item.alternatives.map((alt) => (
                <button
                  key={alt}
                  className="rounded-full bg-card px-2.5 py-1 text-[11px] font-semibold text-ai transition-all active:scale-[0.96]"
                  onClick={() => onMove(item.id, alt)}
                >
                  {alt}
                </button>
              ))}
      ...
      <TimePickerBottomSheet
        ...
        recommendation={item}
        calendarEvents={calendarEvents ?? []}
        onConfirm={(newTime) => onMove(item.id, newTime)}
      />
    </article>
  );
}
...
function ActionBtn({ icon, label, tone = 'default', onClick }) {
  ...
  return (
    <button
      className={`flex flex-1 flex-col items-center gap-0.5 rounded-[14px] py-2 text-center transition-all active:scale-[0.96] ${toneClass}`}
      onClick={onClick}
    >
